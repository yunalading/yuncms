...
CREATE TABLE `yc_article` (
  ...
  `art_keywords` varchar(100) NOT NULL COMMENT '关键词',
  `art_description` varchar(255) NOT NULL COMMENT '描述',
...
CREATE TABLE `yc_category` (
  ...
  `cate_description` varchar(255) NOT NULL DEFAULT '' COMMENT '网站描述',
  `cate_view` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '查看次数',
  `cate_pid` int(5) unsigned zerofill NOT NULL DEFAULT '00000' COMMENT '父级id',
  `cate_order` tinyint(4) unsigned NOT NULL DEFAULT '100' COMMENT '栏目排序',
  `cate_module` varchar(30) NOT NULL DEFAULT '' COMMENT '所属模型',
  `cate_template` varchar(30) NOT NULL COMMENT '栏目列表模板',
  `cate_status` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '0正常显示，1隐藏',
  PRIMARY KEY (`cate_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='网站栏目分类';
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `yc_comment` (
  ...
  `user_id` int(11) unsigned zerofill NOT NULL COMMENT '评论用户的id',
  `user_name` varchar(50) NOT NULL COMMENT '评论用户的名称',
  `art_id` int(11) NOT NULL COMMENT '评论的文章id',
  `content` text NOT NULL COMMENT '评论内容',
  `reply_id` int(11) unsigned zerofill NOT NULL COMMENT '管理员回复人id',
  `reply_msg` text NOT NULL,
  `is_sh` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '是否审核（0未审核1审核通过2审核不通过）',
  `order` int(5) unsigned zerofill NOT NULL DEFAULT '00100' COMMENT '排序',
  PRIMARY KEY (`cid`)
...
CREATE TABLE `yc_config` (
  ...
  `theme` varchar(30) NOT NULL DEFAULT '' COMMENT '后台默认主题',
  `website` varchar(150) NOT NULL DEFAULT '' COMMENT '网站备案信息',
  `copyright` varchar(100) NOT NULL DEFAULT '' COMMENT '版权信息',
  `forbitip` varchar(255) NOT NULL DEFAULT '' COMMENT '禁止访问IP（逗号隔开）',
...
CREATE TABLE `yc_guestbook` (
  ...
  `guest_name` varchar(150) NOT NULL DEFAULT '' COMMENT '姓名',
  `guest_email` varchar(150) NOT NULL COMMENT '邮箱',
  ...
  `guest_ status` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '0正常1删除',
...
CREATE TABLE `yc_links` (
  ...
  `link_logo` varchar(150) NOT NULL DEFAULT '' COMMENT '连接图标',
...
CREATE TABLE `yc_module` (
  ...
  `mname` varchar(100) NOT NULL DEFAULT '' COMMENT '模型名称',
  `module` varchar(100) NOT NULL DEFAULT '' COMMENT '模型英文标识',
  PRIMARY KEY (`mid`)
...
CREATE TABLE `yc_zt` (
  ...
  `zt_title` varchar(100) NOT NULL DEFAULT '' COMMENT '文章标题',
  `zt_keywords` varchar(100) NOT NULL DEFAULT '' COMMENT '关键词',
  `zt_description` varchar(255) NOT NULL DEFAULT '' COMMENT '描述',
  `zt_thumb` varchar(100) NOT NULL DEFAULT '' COMMENT '缩略图',
  `zt_content` text NOT NULL COMMENT '内容',
  `zt_addtime` int(11) unsigned zerofill NOT NULL DEFAULT '00000000000' COMMENT '发布时间',
  `zt_author` varchar(50) NOT NULL DEFAULT '' COMMENT '作者',
  `zt_view` int(11) unsigned NOT NULL DEFAULT '0' COMMENT '查看次数',
  `zt_status` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '0正常显示，1隐藏',
  `zt_order` tinyint(5) unsigned NOT NULL DEFAULT '100' COMMENT '文章排序',
  `zt_module` varchar(255) NOT NULL DEFAULT '' COMMENT '模板指定模型,没指定用上级栏目的',
  `zt_template` varchar(50) NOT NULL DEFAULT '' COMMENT '文章模板',
  PRIMARY KEY (`zt_id`)
